...
def evaluate_full_resolution(
    model, dataset_paths, tile_size=512, overlap=64, device="mps"
):
    """
    ...
    eval_dataset = GoProDataset(
        val_blur_paths, val_sharp_paths, is_train=False, full_image=True
    )
    ...
    for i in tqdm(range(len(eval_dataset)), desc="Evaluating"):
        ...
        prediction = predict_tiled(
            model, blur_input, tile_size=tile_size, overlap=overlap, device=device
        )
